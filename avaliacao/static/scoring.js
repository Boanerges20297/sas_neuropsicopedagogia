// Sistema de Pontuação de Respostas - Compatível com Django CSRF
document.addEventListener('DOMContentLoaded', function() {
    ...
    saveScoreBtn.addEventListener('click', async function() {
        ...
        try {
            // Obter URL a partir do data-url injetado pelo Django ou reconstruir
            let saveUrl = btn.dataset.url;
            if (!saveUrl) {
                const pathParts = window.location.pathname.split('/').filter(p => p !== "");
                const responseId = pathParts[pathParts.length - 1];
                saveUrl = `/admin/resposta/${responseId}/pontuar/`;
            }

            // Capturar o token CSRF obrigatório do Django
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    btn.innerHTML = '<span>💾</span> Salvar Parecer';
                    btn.disabled = false;
                }, 2000);
            } else {
